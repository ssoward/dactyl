...
import { KARMA_DELTAS } from './events.js';
import { newKarmaId } from '../lib/id.js';
export function computeTier(karma) {
    if (karma >= 500)
        return AgentTier.ELITE;
    if (karma >= 200)
        return AgentTier.EXPERT;
    if (karma >= 50)
        return AgentTier.RELIABLE;
 ...
 *   4. Return new values
 */
export async function applyKarmaEvent(agentId, eventType, taskId, pool) {
    ...
    try {
        ...
        const currentKarmaResult = await client.query('SELECT karma FROM agents WHERE id = $1 FOR UPDATE', [agentId]);
        ...
        await client.query(`INSERT INTO karma_events (id, agent_id, event_type, delta, task_id)
       VALUES ($1, $2, $3, $4, $5)`, [karmaId, agentId, eventType, delta, taskId ?? null]);
        // Single UPDATE: karma, tier, and last_active_at all set correctly at once.
        await client.query(`UPDATE agents SET karma = $1, tier = $2, last_active_at = NOW() WHERE id = $3`, [newKarmaValue, newTier, agentId]);
