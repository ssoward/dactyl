...
import { canClaim, resolveClaimDeadlines } from '../tasks/state-machine.js';
import { scheduleClaimExpiry, scheduleKarmaAutoAward, scheduleTaskExpiry, cancelJob, QUEUE_KARMA_AUTO_AWARD, } from '../tasks/expiry.js';
...
// ─── Webhook helper ───────────────────────────────────────────────────────────
async function queueTaskWebhook(agentId, eventType, task) {
    const pool = getPool();
    ...
    await pool.query(`INSERT INTO webhook_deliveries (id, agent_id, event_type, task_id, payload)
     VALUES ($1, $2, $3, $4, $5)`, [deliveryId, agentId, eventType, task.id ?? null, JSON.stringify(payload)]);
    const agentRows = await query('SELECT webhook_url FROM agents WHERE id = $1', [agentId]);
...
export async function taskRoutes(app) {
    // POST /tasks
    app.post('/tasks', { preHandler: authMiddleware, schema: { body: PostTaskBody } }, async (request, reply) => {
        const body = request.body;
        const agentId = request.agentId;
        const pool = getPool();
        // Verify lane exists
        const laneRows = await query('SELECT slug FROM lanes WHERE slug = $1', [body.lane_slug]);
    ...
    app.get('/tasks', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        params.push(limit + 1);
        const tasks = await query(`SELECT * FROM tasks ${where} ORDER BY created_at DESC LIMIT $${paramIdx}`, params);
        let nextCursor;
    ...
    app.post('/tasks/:id/claim', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        const agentId = request.agentId;
        const pool = getPool();
        const taskRows = await query('SELECT * FROM tasks WHERE id = $1', [taskId]);
        ...
        // Fetch agent karma
        const agentRows = await query('SELECT karma FROM agents WHERE id = $1', [agentId]);
        ...
        if (!karmaCheck.ok) {
            throw new DactylError(ERROR_CODES.INSUFFICIENT_KARMA, karmaCheck.detail ?? {});
        }
        // Atomic Redis claim lock
        const acquired = await acquireClaim(taskId, agentId, parseInt(process.env['CLAIM_TTL_SECONDS'] ?? '600', 10));
        if (!acquired) {
       ...
       SET status = 'claimed',
           ...
           updated_at = NOW()
       WHERE id = $5 AND status = 'open'`, [agentId, now, claimExpiresAt, progressDeadlineAt, taskId]);
    ...
    app.post('/tasks/:id/result', { preHandler: authMiddleware, schema: { body: SubmitResultBody } }, async (request, reply) => {
        const { id: taskId } = request.params;
        const agentId = request.agentId;
        const body = request.body;
        const pool = getPool();
        ...
        await query(`UPDATE tasks
         SET status = 'completed',
             ...
             completed_at = $2,
             updated_at = NOW()
         WHERE id = $3`, [JSON.stringify(body.result_payload), now, taskId]);
        // Increment tasks_completed
        await query('UPDATE agents SET tasks_completed = tasks_completed + 1 WHERE id = $1', [agentId]);
        // Check if this is the agent's first completion in this lane (count is
        // now 1 because the UPDATE above just incremented it).
        const laneCompletions = await query(`SELECT COUNT(*) as count FROM tasks
         WHERE claimed_by_agent_id = $1 AND lane_slug = $2 AND status = 'completed'`, [agentId, task.lane_slug]);
        ...
        await query('UPDATE tasks SET karma_auto_award_job_id = $1 WHERE id = $2', [jobId, taskId]);
    ...
    // POST /tasks/:id/vote
    app.post('/tasks/:id/vote', { preHandler: authMiddleware, schema: { body: VoteBody } }, async (request, reply) => {
        const { id: taskId } = request.params;
        const agentId = request.agentId;
        const body = request.body;
        const pool = getPool();
        ...
        await query('UPDATE tasks SET vote = $1, voted_at = NOW(), updated_at = NOW() WHERE id = $2', [body.vote, taskId]);
        ...
        await query('UPDATE tasks SET karma_awarded = $1 WHERE id = $2', [karmaDelta, taskId]);
        if (body.vote === 'up') {
    ...
    app.post('/tasks/:id/abandon', { preHandler: authMiddleware }, async (request, reply) => {
       ...
       WHERE id = $1`, [taskId]);
        ...
        await query('UPDATE agents SET tasks_abandoned = tasks_abandoned + 1 WHERE id = $1', [agentId]);
    ...
    // POST /tasks/:id/boost
    app.post('/tasks/:id/boost', { preHandler: authMiddleware, schema: { body: BoostBody } }, async (request, reply) => {
        const { id: taskId } = request.params;
        const agentId = request.agentId;
        const body = request.body;
        const pool = getPool();
        ...
        const boostedUntil = new Date(Date.now() + body.duration_hours * 60 * 60 * 1000);
        await query(`UPDATE tasks
         SET boosted = TRUE,
             boosted_until = $1,
             updated_at = NOW()
         WHERE id = $2`, [boostedUntil, taskId]);
        return reply.send({
