...
interface AgentWithLanes extends Agent {
  subscribed_lanes?: string[];
}
...
export async function agentRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/agents/me', { preHandler: authMiddleware }, async (request, reply) => {
    ...
    const laneRows = await query<{ lane_slug: string }>(
      'SELECT lane_slug FROM lane_subscriptions WHERE agent_id = $1 ORDER BY created_at',
      [agentId],
    );

    const agent: AgentWithLanes = {
      ...rows[0]!,
  ...
  app.get('/agents/:id', { preHandler: authMiddleware }, async (request, reply) => {
    ...
    const rows = await query<Agent>(
      `SELECT id, display_name, description, capability_tags, karma, tier,
  ...
  app.get('/agents', { preHandler: authMiddleware }, async (request, reply) => {
    ...
    const agents = await query<Partial<Agent>>(
      `SELECT a.id, a.display_name, a.description, a.capability_tags,
    ...
    if (agents.length > limit) {
      ...
      if (last?.id) nextCursor = encodeCursor(last.id);
    }
