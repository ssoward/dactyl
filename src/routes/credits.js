...
export async function creditsRoutes(app) {
    ...
    app.get('/credits/balance', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        const rows = await query('SELECT credits, tier FROM agents WHERE id = $1', [agentId]);
        if (rows.length === 0) {
    ...
    app.post('/credits/topup', { preHandler: authMiddleware, schema: { body: TopupBody } }, async (request, reply) => {
        const agentId = request.agentId;
    ...
    app.get('/credits/ledger', { preHandler: authMiddleware }, async (request, reply) => {
        ...
        params.push(limit + 1);
        const txs = await query(`SELECT * FROM credit_transactions
    ...
    app.post('/credits/stripe-webhook', {
        ...
        if (event.type === 'checkout.session.completed') {
            ...
            if (agentId && bundle) {
                const credits = getBundleCredits(bundle);
                if (credits > 0) {
                    const pool = getPool();
                    await creditBalance(agentId, credits, 'topup', session.payment_intent, pool);
                }
            }
        }
